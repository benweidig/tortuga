...
// CheckForGit checks if git executable is available in the path
...
func git(path string, args ...string) (bytes.Buffer, bytes.Buffer, error) {
...
func LocalBranch(path string) (string, error) {
	stdOut, _, err := git(path, "rev-parse", "--abbrev-ref", "HEAD")
	if err != nil {
		return "", err
	}
...
func UpstreamBranch(path string) (string, error) {
	stdOut, _, err := git(path, "rev-parse", "--symbolic-full-name", "--abbrev-ref", "@{u}")
	if err != nil {
		return "", err
	}
...
func commitsCount(path string, rangeSpecifier string) (int, error) {
	stdOut, _, err := git(path, "rev-list", rangeSpecifier)
	if err != nil {
		return -1, err
	}
